...
// readAuxRekeyState reads both records in one round trip: the given pass's
// in-progress sentinel and the (pass-independent) drift record. A missing
...
//
// The drift record is deliberately NOT per-pass: drift is a property of a
// table's storage, not of the pass that tripped over it, so a table skipped by
// one pass is skipped by both until the drift is repaired.
func readAuxRekeyState(ctx context.Context, db DBConn, sentinelKey string) (auxRekeyState, error) {
	...
	// sentinelKey is the caller's pass sentinel (upstream now runs more than one
	// pass, each with its own), read in the same round trip as the drift record.
...
// readAuxRekeyDrifted reads just the drift record, which is pass-independent.
// MigrateUp needs it before it knows which pass (if any) will run, so it cannot
// name a sentinel — the empty key matches no row in local_metadata, leaving the
// drift half of the query as the only thing that can match.
func readAuxRekeyDrifted(ctx context.Context, db DBConn) ([]string, error) {
	state, err := readAuxRekeyState(ctx, db, "")
	if err != nil {
		return nil, err
	}
	return state.drifted, nil
}
...
func rekeyAuxRowIDsPending(ctx context.Context, db DBConn, mainVersionBefore int, pass auxRekeyPass, pending []int) (bool, error) {
	...
	// NOTE: upstream returns early here on !markerPending. That check is not
	// dropped — it moved into the combined gate below, which also consults the
	// drift record. Returning here would make a drift skip permanent, since the
	// marker is recorded by the very pass that skipped.
