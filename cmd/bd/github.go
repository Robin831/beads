...
func runGitHubRepos(cmd *cobra.Command, args []string) error {
	...
	config := getGitHubConfig()
	if config.Token == "" {
		// Upstream's HandleError wrapper, with the fork's GH_TOKEN fallback
		// named too — autosync accepts it, so the message must mention it.
		return HandleError("github.token is not configured. Set via 'bd config set github.token <token>', or the GITHUB_TOKEN or GH_TOKEN environment variable")
	}
...
func runGitHubSync(cmd *cobra.Command, args []string) error {
	...
	// buildGitHubEngine is the fork's extraction of exactly the setup upstream
	// still inlines here. It has to stay a function: the autosync path builds
	// the same engine, and duplicating it is how the two drift apart.
