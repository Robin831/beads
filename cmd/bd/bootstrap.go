...
func cloneViaEmbedded(ctx context.Context, beadsDir, remoteURL, dbName string) error {
	...
	if err := versioncontrolops.DoltClone(ctx, db, remoteURL, dbName, strings.TrimSpace(os.Getenv("DOLT_REMOTE_USER"))); err != nil {
...
func cloneViaServer(ctx context.Context, beadsDir, remoteURL, dbName string, cfg *configfile.Config) error {
	...
	if err := versioncontrolops.DoltClone(cloneCtx, db, remoteURL, dbName, strings.TrimSpace(os.Getenv("DOLT_REMOTE_USER"))); err != nil {
